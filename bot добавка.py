...
def is_admin(uid):
    return uid == ADMIN_ID or uid in admin_users
waiting_pin = set()
...
# поэтому бот открывает один простой read-only эндпоинт с остатками.
...
async def run_web_server():
    ...
    await site.start()
    logging.info(f"HTTP API запущен на порту {PORT} (GET /stock)")
...
async def daily_report_task():
    """Раз в сутки, в REPORT_HOUR по Ташкенту, шлёт отчёт админу."""
    while True:
