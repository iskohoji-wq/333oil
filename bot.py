...
async def client_status(msg: Message):
    ...
    if lang == 'uz':
        status_map = {
            ...
            'enroute': "🚗 Haydovchi yo'lda",
            'done': '✅ Bajarildi',
            'cancelled': '❌ Bekor qilindi'
        }
        status = status_map.get(last['status'], '—')
        await msg.answer(
            f"📋 <b>Buyurtma #{last['id']}</b>\n\n"
            f"⛽ {last['fuel']} — {last['liters']} l\n"
            f"💰 {last['total']:,} so'm\n"
            f"🔄 {status}"
        )
    else:
...
async def client_history(msg: Message):
    ...
    if not user_orders:
        ...
        return
    if lang == 'uz':
        lines = [f"#{o['id']} • {o['fuel']} {o['liters']}l • {o['total']:,} so'm" for o in reversed(user_orders[-5:])]
        await msg.answer("📋 <b>Tarix:</b>\n\n" + "\n".join(lines))
    else:
